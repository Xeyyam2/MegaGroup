...
| **P1** | Ölkə alt-səhifə siloları (universitetler, tibb, attestatla-qebul, tehsil-haqqi, teqaud, magistr, yasayis-xercleri) | Kod + məzmun | ✅ 2026-09-05 (7 topic × 7 ölkə × 3 dil; magistr 2026-09-05 əlavə olundu) |
...
| 2026-09-05 | `src/data/country-topics.ts` (+ magistr), `src/app/[locale]/xaricde-tehsil/[country]/magistr/page.tsx` (YENİ), `CountryTopicPage.tsx`, `src/data/faqs.ts`, `src/data/testimonials.ts`, `src/lib/data/faqs.ts`, `src/lib/data/testimonials.ts`, ölkə/bloq səhifələri | 1.2 + 2.1 (P1) | **Magistratura silo topic-i əlavə olundu** (əvvəl yox idi): 7 ölkə × 3 dil = 21 yeni səhifə (`/magistr`), hər dil üçün title/meta/intro + sənəd siyahısı + addım bələdçisi (bakalavr cədvəli magistr səhifəsində göstərilmir). Ölkə + bloq "Əlaqəli səhifələr" nav çiplərinə magistr etiketi əlavə olundu; sitemap avtomatik 21 URL-i əhatə edir. **Yolda tapılan canlı bug:** statik fallback-da FAQ (sual/cavab) və testimonial sitatları locale-dən asılı olmayaraq AZ qaytarırdı → RU/EN səhifələrində AZ suallar/sitatlar görünürdü (məs. EN səhifədə "DIM imtahanı mütləqdir?"). Statik getterlər locale-aware edildi (Supabase mapFaqRow/mapTestimonialRow ilə eyni qayda); RU/EN render yoxlanıldı — AZ sızıntı yoxdur | `tsc --noEmit` ✅, lint ✅, `next build` ✅ |
...
| 2026-09-05 | `supabase/migrations/0009_faculties_real.sql` (YENİ), canlı Supabase `faculties`, `src/data/universities.ts`, `src/lib/localize-language.ts` (YENİ), universitet/silo/ixtisas səhifələri | 2.2 + 9 (P0-1) | **Real fakültə/proqram məlumatı (63 qeyd, 14 universitet)**: əvvəlki sıx AZ-only qeydlər kanonik dəstlə əvəz olundu — hər fakültə az/ru/en adı, real müddət və dil (rəsmi normlar üzrə: Almaniya bakalavrı 3 il — əvvəl səhv 4; "İnformatika" TUM-da alman dilindədir). Uydurma qeydlər silindi (Al-Farabi"də tibb yox idi), faktiki olmayan proqramlar əlavə olunmadı (yalnız yoxlanılmış). Kiyev Tibb Universitetinin rəsmi saytı kmu.edu.ua-ya düzəldildi. Fakültə cədvəllərində dil tokenləri RU/EN səhifələrdə lokallaşdırıldı (Türkçe→Турецкий/Turkish). Statik fallback (`universities.ts`) DB ilə eyniləşdirildi. Lokal yoxlama qeydi: bu maşının `.env.local`-ında anon key olmadığı üçün lokal statik fallback üzərində yoxlanıldı — prod hostunda DB qeydləri göstərilir | `tsc --noEmit` ✅, lint ✅, `next build` ✅ |
